...
public class CursoViewController {
	...
	//Lista de almacenamiento de cursos
	List<Curso> cursosFiltrados = new ArrayList<Curso>();
	
	


	//Resteasy poeticiones HTTP GET,POST,PUT,DELETE
	...
	public void guardar() throws Exception {
		...
		//Convertimos  ajson lo recivido en la vista
		ObjectMapper mapper = new ObjectMapper();
		...
		if(cursoInsertado.getId() > 0) {
			...
			mensaje ="Error al insertar el curso, intenete mas tarde";
		}
		
		FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO,
				"Guardar Curso",mensaje);
